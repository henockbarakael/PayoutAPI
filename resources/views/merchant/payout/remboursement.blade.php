            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                            ...
                            <form  action="{{route('merchant.remboursement.import')}}" method="POST" enctype="multipart/form-data">
                                @csrf
                            ...
                            <tbody class="list form-check-all">
                                ...
                                <tr id="tr_{{$item->id}}">
                                    ...
                                    <td class="text-center">
                                        {!! Form::open(['method' => 'POST','route' => ['merchant.remboursement', $item->id],'style'=>'display:inline']) !!}
                                        {!! Form::submit('Payer', ['class' => 'btn btn-secondary btn-border btn-sm','data-toggle'=>'confirmation','data-placement'=>'left']) !!}
                                        {!! Form::close() !!}

                                        {!! Form::open(['method' => 'DELETE','route' => ['merchant.remboursement.delete', $item->id],'style'=>'display:inline']) !!}
        ...
        $(document).ready(function () {
            ...
            $('.pay-all').on('click', function(e) {
                ...
                // Start Multiple Paiement
                    ...
                    $.ajax({
                        url: "{{ route('merchant.remboursement.multiple') }}",
                        type: 'POST',
            ...
            $('.delete-all').on('click', function(e) {
                ...
                if(idsArr.length <=0)  {  
                    ...
                    if(confirm("Are you sure, you want to delete the selected transactions?")){  
                        var strIds = idsArr.join(","); 
                        $.ajax({
                            url: "{{ route('merchant.remboursement.delete.multiple') }}",
                            type: 'DELETE',
                            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                            data: 'ids='+strIds,
                            success: function (data) {
