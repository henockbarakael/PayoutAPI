            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                            ...
                            <form  action="{{route('merchant.import.test')}}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="row g-3">
                                <div class="col-lg-8">
                    ...
                    <div class="card-body">
                        <table class="display table table-bordered" id="fixed-header" style="width:100%">
                            ...
                            <tbody class="list form-check-all">
                                @if($payout_tests->count())
                                @foreach ($payout_tests as $key => $item)
                                <tr id="tr_{{$item->id}}">
                                    <th scope="row">
                                        <div class="form-check">
                                    ...
                                    <td class="text-center">
                                        {!! Form::open(['method' => 'POST','route' => ['merchant.paiement', $item->id],'style'=>'display:inline']) !!}
                                        {!! Form::submit('Payer', ['class' => 'btn btn-secondary btn-border btn-sm','data-toggle'=>'confirmation','data-placement'=>'left']) !!}
                                        {!! Form::close() !!}

                                        {!! Form::open(['method' => 'DELETE','route' => ['merchant.paiement.delete', $item->id],'style'=>'display:inline']) !!}
